...
- `null` or `{}`: Default heuristics apply.
- `{`... `"url_pattern": ` _pattern_ ...`}`: A desired upload location can be derived by [Pattern Expansion](#pattern-expansion) on _pattern_.
...
- `{`... `"sha256": ` _column_ | `True` ...`}`: If _column_, then the _column_ stores the checksum generated by the 'sha256' cryptographic hash function. It MUST be ASCII/UTF-8 hexadecimal encoded. If `True`, then the client SHOULD generate a 'sha256' checksum and communicate it to the asset storage service according to its protocol.

Default heuristics:
- The annotation explicitly indicates that the associated column is the asset location.
- If no `url_pattern` is specified, client decisions about how to form the upload URL must be decided by other policies.
