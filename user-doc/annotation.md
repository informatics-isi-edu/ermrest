...
- `{`... `"url_pattern": ` _pattern_ ...`}`: A desired upload location can be derived by [Pattern Expansion](#pattern-expansion) on _pattern_. This attribute is required and if it is not specified the asset annotation will be ignored. See implementation notes below.
